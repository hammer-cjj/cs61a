...
def distance(city1, city2):
    ...
    5.0
    """
    lat_distance = abs(get_lat(city1) - get_lat(city2))
    lon_distance = abs(get_lon(city1) - get_lon(city2))
    return sqrt(lat_distance * lat_distance + lon_distance * lon_distance)
...
def closer_city(lat, lon, city1, city2):
    ...
    c1_c3 = distance(city1, city3)
    c2_c3 = distance(city2, city3)
    if c1_c3 < c2_c3:
...
def sprout_leaves(t, values):
    ...
    """
    if is_leaf(t):
    	return tree(label(t), [tree(_) for _ in values])
    else:
